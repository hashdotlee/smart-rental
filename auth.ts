			...
			async authorize(credentials) {
				...
				try {
					...
					if (credentials.isAdmin === 'true') {
						const isAdmin = authData.record.isAdmin === true;
						if (!isAdmin) {
					...
					return {
						...
						isAdmin: authData.record.isAdmin || false, // Thêm isAdmin vào user
					};
				} catch (error) {
	...
	callbacks: {
		async jwt({ token, account, user, profile }) {
			...
			if (user) {
				...
				token.isAdmin = user.isAdmin || false; // Thêm isAdmin vào token
			}
		...
		async session({ session, token }) {
			...
			if (token) {
				...
				session.user.isAdmin = token.isAdmin as boolean || false; // Thêm isAdmin vào session
